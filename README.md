...
```bash
# Uses configs/example.json by default
python example_tts.py --setup
...
3. **Run with your client**:
   ```bash
   python example_tts.py --client new_client
   ```

### Available Configurations

- `configs/example.json` - MAHAJAK brand configuration (default)
...
```bash
# Default (uses configs/example.json)
python example_tts.py

# Specify client config
python example_tts.py --client example

# Custom config file
python example_tts.py --config /path/to/config.json

# Override config values via CLI
python example_tts.py --client example --voice "Custom Voice" --max-scripts 5

# Or specify CSV explicitly
python example_tts.py --csv /path/to/other.csv

# Start from version 5, process only 3 versions
python example_tts.py --start-version 5 --limit 3

# Run in headless mode (no browser window)
python example_tts.py --headless

# Dry run mode (fill forms but skip Generate Speech)
python example_tts.py --dry-run

# No-save mode (fill and generate but skip Save button)
python example_tts.py --no-save

# Debug mode (keep browser open after execution)
python example_tts.py --debug
```
...
12:34:56 | INFO | ======================================================================
12:34:57 | INFO | 📋 Loaded config: configs/example.json
...
```bash
python example_tts.py --csv MAHAJAK_SCRIPT.csv
```
...
Detailed logs are saved to `logs/example_TIMESTAMP.log`
...
1. **Default behavior**: Runs with `configs/example.json` which has the same values as old hardcoded constants
