...
python example_tts.py --csv /path/to/file.csv
```
...
python example_tts.py --voice "Custom Voice" --max-scripts 5
python example_tts.py --base-url "https://app.anylive.jp/scripts/XXX"
python example_tts.py --template "Template_Name"
```
...
- Default config: `configs/example.json`
...
### Key Files
- `example_tts.py`: Main script (single-file design for future app compilation)
...
3. Run: `python example_tts.py --client new_client`
...
2. Check `logs/example_*.log` for detailed DEBUG-level logs
